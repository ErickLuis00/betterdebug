...
// --- Added: Framework Support Definition ---
interface SupportedFramework {
	name: string;
	configFiles: string[];
	wrapperPath: string; // Relative path within the extension's 'out' directory
}
...
const supportedFrameworks: SupportedFramework[] = [
	{
		...
		wrapperPath: '/out/frameworks/nextjs.js'
	},
	// Add more frameworks here in the future
	// e.g., { name: 'Vite', configFiles: ['vite.config.js', 'vite.config.ts'], setupPath: '/out/frameworks/vite.js' }
];
	...
	for (let i = history.length - 1; i >= 0; i--) {
		...
		if (isNewTimestampGroup) {
			...
			if (groupType === 'call-result' && entry.name) {
				md.appendMarkdown(`##### **${time}** (${timeAgo}) - Call: \`${entry.name}\` [${entry.env}]`);
			} else {
				md.appendMarkdown(`##### **${time}** (${timeAgo}) - Type: \`${groupType}\` [${entry.env}]`);
			}
		}
...
// --- BetterDebug Setup Function (Refactored for Modularity) ---
// Returns true if setup is successful for a supported project, false otherwise.
async function ensureBetterDebugSetup(context: vscode.ExtensionContext): Promise<boolean> {
	const workspaceFolder = vscode.workspace.workspaceFolders?.[0]?.uri.fsPath;
	if (!workspaceFolder) {
		console.error('[BetterDebug Setup] No workspace folder open. Cannot check for project type or perform setup.');
		return false; // Cannot proceed without a workspace
	}

	let detectedFramework: SupportedFramework | null = null;

	// Iterate through supported frameworks to detect the project type
	for (const framework of supportedFrameworks) {
		for (const configFile of framework.configFiles) {
			const configFilePath = path.join(workspaceFolder, configFile);
			try {
				await fsp.access(configFilePath);
				detectedFramework = framework;
				console.log(`[BetterDebug Setup] Detected ${framework.name} project (found ${configFile}).`);
				break; // Found a config file for this framework, no need to check others for it
			} catch {
				// File doesn't exist, continue checking
			}
		}
		if (detectedFramework) {
			break; // Found a supported framework, stop searching
		}
	}

	// If no supported framework was detected
	if (!detectedFramework) {
		vscode.window.showErrorMessage('BetterDebug: Support for this project type is currently WIP.');
		console.log('[BetterDebug Setup] No supported framework detected. Skipping setup.');
		return false; // Indicate unsupported project
	}

	// --- Proceed with setup ONLY for the detected supported framework ---
	console.log(`[BetterDebug Setup] Ensuring .betterdebug setup for ${detectedFramework.name}...`);
	const extensionPath = context.extensionPath;
	const formattedExtensionPath = extensionPath.replace(/\\/g, '/');
	const betterDebugDir = path.join(workspaceFolder, '.betterdebug');
	const betterDebugIndexFile = path.join(betterDebugDir, 'index.js');

	// Dynamically generate the require path based on the detected framework
	const requiredContent = `// ⚠️ DO NOT CHANGE OR REMOVE THIS FILE, IT IS AUTO GENERATED BY THE BETTERDEBUG EXTENSION
const extensionPath = '${formattedExtensionPath}'
module.exports = require(extensionPath + '${detectedFramework.wrapperPath}').default({ extensionPath })
// ⚠️ DO NOT CHANGE OR REMOVE THIS FILE, IT IS AUTO GENERATED BY THE BETTERDEBUG EXTENSION
`;

	try {
		// 1. Ensure .betterdebug directory exists
		try {
			await fsp.access(betterDebugDir);
		} catch (dirError: any) {
			if (dirError.code === 'ENOENT') {
				console.log(`[BetterDebug Setup] Creating directory: ${betterDebugDir}`);
				await fsp.mkdir(betterDebugDir);
			} else {
				throw dirError; // Re-throw unexpected errors
			}
		}

		// 2. Check and write/update index.js
		let existingContent = '';
		try {
			existingContent = await fsp.readFile(betterDebugIndexFile, 'utf8');
		} catch (fileError: any) {
			if (fileError.code !== 'ENOENT') {
				throw fileError; // Re-throw unexpected errors
			}
			// File doesn't exist, which is fine, we'll create it
			console.log(`[BetterDebug Setup] File not found, will create: ${betterDebugIndexFile}`);
		}

		if (existingContent !== requiredContent) {
			console.log(`[BetterDebug Setup] Writing/Updating ${betterDebugIndexFile}`);
			await fsp.writeFile(betterDebugIndexFile, requiredContent, 'utf8');
		} else {
			console.log(`[BetterDebug Setup] File already up-to-date: ${betterDebugIndexFile}`);
		}

		return true; // Setup successful or already done for a supported project

	} catch (error) {
		console.error(`[BetterDebug Setup] Error ensuring setup in ${workspaceFolder}:`, error);
		vscode.window.showErrorMessage(`BetterDebug Setup failed: ${error instanceof Error ? error.message : String(error)}`);
...
export async function activate(context: vscode.ExtensionContext) {
	...
	// Ensure the .betterdebug folder and index.js are set up
	// Exit activation if the project is not supported or setup fails.
	const setupSuccessful = await ensureBetterDebugSetup(context);
	if (!setupSuccessful) {
		console.log('[BetterDebug Activation] Setup was not successful or project not supported. Aborting activation.');
		return; // Stop activation
	}

	console.log('[BetterDebug Activation] Setup successful. Proceeding with activation...');

	// Set up listener for workspace folder changes (only relevant if initial setup was successful)
	context.subscriptions.push(
		vscode.workspace.onDidChangeWorkspaceFolders(async event => {
			// Check if folders were added and there's now at least one folder
			if (event.added.length > 0 && vscode.workspace.workspaceFolders && vscode.workspace.workspaceFolders.length > 0) {
				console.log('[BetterDebug Setup] Workspace folders changed, ensuring setup...');
				await ensureBetterDebugSetup(context);
			}
		})
	);
	...
	function startServer() {
		...
		try {
			...
			vscode.window.showInformationMessage(`BetterDebug started on port ${port}.`);
			...
			wss.on('error', (error) => {
				...
				vscode.window.showErrorMessage(`BetterDebug error: ${error.message}`);
			...
			if (!wss) {
				vscode.window.showErrorMessage(`Failed to start BetterDebug on port ${port}. Is the port in use?`);
			}
			wss = null;
			setTimeout(startServer, 10000);
		}
